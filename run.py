...
topo2bdp = {
    ...
    "leaf_spine_256_100G_OS1": 104000,  # 2-tier -> all 100Gbps
    "leaf_spine_256_100G_OS2": 104000,  # 2-tier -> all 100Gbps
    "leaf_spine_512_100G_OS1": 104000,  # 2-tier -> all 100Gbps
    "leaf_spine_512_200G_OS1": 204000,  # 2-tier -> all 100Gbps
...
def main():
    ...
    parser.add_argument('--lb', dest='lb', action='store',
                        default='gemma', help="fecmp/pecmp/drill/conga (default: fecmp)")
    ...
    parser.add_argument('--simul_time', dest='simul_time', action='store',
                        default='0.01', help="traffic time to simulate (up to 3 seconds) (default: 0.1)")
    ...
    parser.add_argument('--netload', dest='netload', action='store', type=int,
                        default=90, help="Network load at NIC to generate traffic (default: 40.0)")
    ...
    parser.add_argument('--topo', dest='topo', action='store',
                        default='leaf_spine_128_100G_OS2', help="the name of the topology file (default: leaf_spine_128_100G_OS2/fat_k4_100G_OS2)")
    ...
    if (exists(os.getcwd() + "/config/" + flow + ".txt")):
        ...
        #     time=args.simul_time,
        #     output=os.getcwd() + "/config/" + flow + ".txt"))
        os.system("python ./traffic_gen/traffic_gen_incast.py -c {cdf} -n {n_host} -l {load} -b {bw} -t {time} -o {output}".format(
            cdf=os.getcwd() + "/traffic_gen/" + args.cdf + ".txt",
            n_host=n_host,
            load=hostload / 100.0,
            bw=args.bw + "G",
    ...
    if lb_mode == 9 or lb_mode == 10: # ConWeave Logging
        ################################################################
