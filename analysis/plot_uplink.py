...
def main():
    ...
    for k, v in map_key_to_id.items():
        ...
        ax.set_xlabel("吞吐量不平衡度", fontproperties=zhfont, fontsize=15.5)
        ax.set_ylabel("累积分布函数", fontproperties=zhfont, fontsize=15.5)
                    ...
                    with open(filename_uplink, "r") as f:
                        ...
                        for switch_id, vvvv in switch_diff_data.items():
                            ...
                            for vec in v_t:
                                if np.average(vec) == 0:
                                    continue
                                val = (np.max(vec) - np.min(vec)) / np.average(vec)
        ...
        fig_filename = fig_dir + "/{}.svg".format("CDF_UPLINK_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
        plt.savefig(fig_filename, transparent=False, bbox_inches='tight',dpi=300)
        plt.close()
            
        ################## Switch Absolute plotting ##################
        fig = plt.figure(figsize=(5, 3))
        ax = fig.add_subplot(111)
        fig.tight_layout()

        ax.set_xlabel("吞吐量 (B/s)", fontproperties=zhfont, fontsize=15.5)
        ax.set_ylabel("累积分布函数", fontproperties=zhfont, fontsize=15.5)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.yaxis.set_ticks_position('left')
        ...
        for tgt_lbmode in lbmode_order:
            for vv in v:
                ...
                if lb_mode == tgt_lbmode:
                    ...
                    for i in range(1):
                        # plotting
                        filename_uplink = output_dir + "/{id}/{id}_out_throughput.txt".format(id=config_id)
                        port_list = set()
                        ...
                        with open(filename_uplink, "r") as f:
                            ...
                            if i == 0:
                               ...
                               ax.plot([x[0] for x in cdf_ts_data_arr],
                                        ...
                                        markersize=0,
                                        linewidth=3.0,
                                        label="{}-all".format(lb_mode)) 
        
        ax.legend(frameon=False, fontsize=12, facecolor='white')
        
        ax.grid(which='minor', alpha=0.2)
        ax.grid(which='major', alpha=0.5)
        fig_filename = fig_dir + "/{}.svg".format("CDF_Throughput_Effective_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
        plt.savefig(fig_filename, transparent=False, bbox_inches='tight',dpi=300)
        plt.close()

        ################## Switch All Absolute plotting ##################
        fig = plt.figure(figsize=(5, 3))
        ax = fig.add_subplot(111)
        fig.tight_layout()

        ax.set_xlabel("Throughput All (Byte/s)", fontsize=15.5)
        ax.set_ylabel("CDF", fontsize=15.5)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.yaxis.set_ticks_position('left')
        ax.xaxis.set_ticks_position('bottom')
        ...
        ax.grid(which='minor', alpha=0.2)
        ax.grid(which='major', alpha=0.5)
        fig_filename = fig_dir + "/{}.svg".format("CDF_Throughput_All_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
        plt.savefig(fig_filename, transparent=False, bbox_inches='tight',dpi=300)
        plt.close()
        ...
        # ax.set_xlabel("Flow Size (Byte)", fontsize=11.5)
