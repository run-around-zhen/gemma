...
def main():
        ...
        for line in f.readlines():
            for topo in topo2bdp.keys():
                if topo in line:
                    ...
                    if lb_mode != "Conweave" and lb_mode != "Gemma":
                        continue
                    if lb_mode == "Conweave":
                        lb_mode = "ConWeave" 
    ...
    for k, v in map_key_to_id.items():
        ...
        ax.set_xlabel("内存使用(KB)", fontproperties=zhfont, fontsize=12.5)
        ax.set_ylabel("累积分布函数", fontproperties=zhfont, fontsize=12.5)
        ...
        for vv in v:
            ...
            print(config_id, np.average(data_voq_volume["x"]))
        ...
        fig_filename = fig_dir + "/{}.svg".format("CDF_QUEUE_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
    ...
    for k, v in map_key_to_id.items():
        ...
        fig = plt.figure(figsize=(4, 2.8))
        ax = fig.add_subplot(111)
        ax.set_xlabel("队列数量使用", fontproperties=zhfont, fontsize=12.5)
        ax.set_ylabel("累积分布函数", fontproperties=zhfont, fontsize=12.5)
        
        for vv in v:
            config_id = vv[0]
            lb_mode = vv[1]
            filename_voq_volume = output_dir + "/{id}/{id}_out_voq_queue_cdf.txt".format(id=config_id)
            data_voq_volume = {"x": [], "y": []}
            with open(filename_voq_volume, "r") as f:
                for line in f.readlines():
                    parsed_line = line.replace("\n","").split(" ")
        ...
        ax.grid(which='minor', alpha=0.2)
        ax.grid(which='major', alpha=0.5)
        fig_filename = fig_dir + "/{}.svg".format("CDF_QUEUE_NUM_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
        plt.savefig(fig_filename, transparent=False, bbox_inches='tight', dpi=300)
        plt.close()
    ...
    for k, v in map_key_to_id.items():
        ...
        fig = plt.figure(figsize=(4, 2.8))
        ax = fig.add_subplot(111)
        ax.set_xlabel("队列使用数量", fontproperties=zhfont, fontsize=12.5)
        ax.set_ylabel("累积分布函数", fontproperties=zhfont, fontsize=12.5)
        
        for vv in v:
            config_id = vv[0]
            lb_mode = vv[1]
            filename_voq_volume = output_dir + "/{id}/{id}_out_voq_per_dst_queue_cdf.txt".format(id=config_id)
            data_voq_volume = {"x": [], "y": []}
            with open(filename_voq_volume, "r") as f:
                for line in f.readlines():
                    parsed_line = line.replace("\n","").split(" ")
        ...
        ax.grid(which='minor', alpha=0.2)
        ax.grid(which='major', alpha=0.5)
        fig_filename = fig_dir + "/{}.svg".format("CDF_QUEUE_NUM_PER_DST_TOPO_{}_LOAD_{}_FC_{}".format(k[0], k[1], k[2]))
        print(fig_filename)
        plt.savefig(fig_filename, transparent=False, bbox_inches='tight', dpi=300)
        plt.close()        
