...
if __name__ == "__main__":
	...
	print(avg)
	avg_inter_arrival = 1/(bandwidth*(load-0.025)/8./avg)*1000000000
	avg_incast = 1/(bandwidth*0.025/8./(200*1000*50))*1000000000/125
	#print(avg_inter_arrival)
	print(avg_incast)
	n_flow_estimate = int(time / avg_inter_arrival * nhost)
	n_flow_incast = int(time/avg_incast )* 50
	print(int(time/avg_incast )* 50)
	print(n_flow_estimate)
	n_flow = 0
	ofile.write("%d \n"%(n_flow_estimate+n_flow_incast))
	# ofile.write("%d \n"%(n_flow_estimate))
	host_list = [(base_t + int(poisson(avg_inter_arrival)), i) for i in range(nhost)]
	heapq.heapify(host_list)
	queuecount = [1 for i in range(1,321)]
	baset = base_t
	while len(host_list) > 0:

		t,src = host_list[0]
		inter_t = int(poisson(avg_inter_arrival))
		if t+avg_inter_arrival-baset>=avg_incast:
			#print(t)
			...
			for i in range(1,51):
				while 1:
					...
					if src != dst:
						break
				# ofile.write("%d %d %d 100 %d %.9f\n"%(src, dst,queuecount[src]%7+1, 200000, t * 1e-9))
				ofile.write("%d %d 3 %d %.9f\n"%(src, dst, 200000, t * 1e-9))
			baset=t+avg_inter_arrival
			n_flow+=50
			# print("incast happen")
		...
		if (t + inter_t > time + base_t):
			...
			if size <= 0:
				size = 1
			n_flow += 1
			queuecount[src] = queuecount[src]+1
			# queuecount[dst] = queuecount[dst]+1
			# ofile.write("%d %d %d 100 %d %.9f\n"%(src, dst, random.randint(1,15),size, t * 1e-9))
			ofile.write("%d %d 3 %d %.9f\n"%(src, dst, size, t * 1e-9))
			# ofile.write("%d %d %d 100 %d %.9f\n"%(src, dst,queuecount[src]%7+1, size, t * 1e-9))
