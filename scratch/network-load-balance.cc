...
void ScheduleFlowInputs(FILE *infile) {
    ...
    while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()) {
         ...
         */
        if (1) {  // logging input streams to "XXXX_out_in.txt"
            /************************
...
void periodic_monitoring(FILE *fout_voq, FILE *fout_voq_detail, FILE *fout_uplink, FILE *fout_conn,
    ...
    for (const auto &tor2If : torId2UplinkIf) {  // for each TOR switches
        ...
        if (lb_mode_val == 9) {  // Conweave
            ...
            for (auto voq : swNode->m_mmu->m_conweaveRouting.GetVOQMap()) {
                auto &nvoq_npkt = dip_to_nvoq_npkt[voq.second.getOutPort()];
                nvoq_npkt.first += 1;
            ...
            // swNode->m_mmu->m_conweaveRouting.GetNumVOQ();
        }
        if (lb_mode_val == 10) {
            uint32_t nVOQ = swNode->m_mmu->m_gemmaRouting.GetNumVOQ();
            uint32_t nVolumeVOQ = swNode->m_mmu->m_gemmaRouting.GetVolumeVOQ();
            fprintf(fout_voq, "%lu,%u,%u,%u\n", now, tor2If.first, nVOQ, nVolumeVOQ);
            vector<uint32_t> ports = tor2If.second;
            for (int i = 0; i < ports.size(); i++) {  // 遍历获取核心交换机的队列信息
                uint32_t aggId = Settings::IdPortToId[GetSwitchPortKey(tor2If.first, ports[i])];
                Ptr<Node> node = n.Get(aggId);    // tor id
                auto aggNode = DynamicCast<SwitchNode>(node);
                if (used[aggId] == 0) {
                    used[aggId] = 1;
                    auto m_aggGemmaRouting = aggNode->m_mmu->m_gemmaRouting;
                    fprintf(fout_voq, "%lu,%u,%u,%u\n", now, aggId, m_aggGemmaRouting.GetNumVOQ(), m_aggGemmaRouting.GetVolumeVOQ());
                    std::unordered_map<uint32_t, std::pair<uint32_t, uint32_t>> dip_to_nvoq_npkt;
                    for (auto voq : m_aggGemmaRouting.GetVOQMap()) {
                        auto &nvoq_npkt = dip_to_nvoq_npkt[voq.second.getOutPort()];
                        nvoq_npkt.first += 1;
                        nvoq_npkt.second += voq.second.getQueueSize();
                    }
            ...
            // monitor VOQ per destination IP <time, dstip, #VOQ, #Pkts>
            std::unordered_map<uint32_t, std::pair<uint32_t, uint32_t>> dip_to_nvoq_npkt;
            for (auto voq : swNode->m_mmu->m_gemmaRouting.GetVOQMap()) {
                auto &nvoq_npkt = dip_to_nvoq_npkt[voq.second.getOutPort()];
                nvoq_npkt.first += 1;
                nvoq_npkt.second += voq.second.getQueueSize();
            }
        ...
        for (const auto &iface : tor2If.second) {
            ...
            uint64_t all_uplink_txbyte = swNode->GetAllBytesOutDev(iface);
            fprintf(fout_uplink, "%lu,%u,%u,%lu,%lu\n", now, tor2If.first, iface, uplink_txbyte, all_uplink_txbyte);
        }
    }
    ...
    for (uint32_t i = 0; i < Settings::node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            for (uint32_t j = 0; j < Settings::node_num; j++) {
                if (i == j) continue;
                Ptr<Node> sw1 = n.Get(i);
                Ptr<Node> sw2 = n.Get(j);
                if (nbr2if[sw1].find(sw2) == nbr2if[sw1].end()) continue;
                uint32_t outPort = nbr2if[sw1][sw2].idx;
                auto swNode = DynamicCast<SwitchNode>(sw1);
                uint64_t uplink_txbyte = swNode->GetTxBytesOutDev(outPort);
                uint64_t all_uplink_txbyte = swNode->GetAllBytesOutDev(outPort);
                fprintf(throughput_output, "%lu,%u,%u,%lu,%lu\n", now, i, outPort, uplink_txbyte, all_uplink_txbyte);
            }
        }
    }
...
void outPutHistory() {
    for (auto i = nextHop.begin(); i != nextHop.end(); i++) {  // every node
        ...
        if (nodeSrc->GetNodeType() == 0) continue;
        Ptr<SwitchNode> swSrc = DynamicCast<SwitchNode>(nodeSrc);
        if (swSrc->m_isToR) continue;
        map<int,int> used;
        ...
        for (auto j = table1.begin(); j != table1.end(); j++) {
            ...
            for (auto next1 : nexts1) {
                uint32_t outPort = nbr2if[nodeSrc][next1].idx;
                if (used[outPort]) continue;
                used[outPort] = 1;
                cout << nodeSrc->GetId() << " " << outPort << " " << DynamicCast<SwitchNode>(next1)->m_isToR << " " << swSrc->GetTxBytesOutDev(outPort) << " " << swSrc->GetAllBytesOutDev(outPort) << std::endl;
            }
        }
    }
}
...
void conga_history_print() {
    ...
    std::cout << "Number of flowlet's timeout:" << CongaRouting::nFlowletTimeout << std::endl
              << "总共的数据包为:" << CongaRouting::nPacketNum << std::endl
              << "Conga's timeout: " << conga_flowletTimeout << std::endl
              << "总共的flowlet数量为:" << CongaRouting::nFlowletNum << std::endl
              << "直接碰到暂停端口的数量为:" << CongaRouting::nPauseFlowletNum << std::endl
              << "过程中被暂停端口的数量为:" << CongaRouting::nPauseFollowNum << std::endl;
}
...
void conweave_history_print() {
    ...
    std::cout << "Number of INIT's Reply sent (RTT_REPLY):" << ConWeaveRouting::m_nReplyInitSent
              ...
              << "\nNumber of MaxRepeatNum:" << Settings::maxRepeatFlowNum
              << std::endl;
...
void gemma_history_print() {
    std::cout << "\n------------Gemma History---------------" << std::endl;
    std::cout << "路径变化的次数为:" << GemmaRouting::m_RankChangeNum
              << "\n收到响应的次数为:" << GemmaRouting::m_ReviceReplyNum
              << "\n端口被暂停的次数为:" << GemmaRouting::m_PortPausedNum
              << "\n重新路由的次数为:" << GemmaRouting::m_RerouteNum
              << "\n包的个数为:" << GemmaRouting::m_packetNum
              << "\n最大的梯度为:" << GemmaRouting::m_maxRank
              << "\n乱序数据包数量为:" << GemmaRouting::m_outOfOrderPkts
              << "\necn标记通知的数量为:" << GemmaRouting::m_nNotifySent
              << "\n无效的重新路由数量为:" << GemmaRouting::m_unusedRerouted
              << std::endl;
    std::cout << "记录的流的大小为: " <<  GemmaRouting::m_RankChangeMap.size() << std::endl;
    auto it = GemmaRouting::m_RankChangeMap.begin();
    while (it != GemmaRouting::m_RankChangeMap.end()) {
        if (it->second == 2) {
    ...
    // }
    std::cout << "--------------------------" << std::endl;
    std::cout << "Gemma finish" << std::endl;
}

uint64_t GetFlowKey(uint32_t ip1, uint32_t ip2, uint16_t port1, uint16_t port2) {
    /** IP_ADDRESS: 11.X.X.1 */
...
void qp_finish(FILE *fout, Ptr<RdmaQueuePair> q) {
    ...
    uint64_t flowKey = GetFlowKey(q->sip.Get(), q->dip.Get(), q->sport, q->dport);
    GemmaRouting::m_RankChangeMap[flowKey] = 1;
    // if (flowKey == 169550556293) {
    //     std::cout << "指定的流完成了" << std::endl;
    // }
    if (Settings::cnt_finished_flows % 1000 == 0 || Settings::cnt_finished_flows > 90000) {
    // if (Settings::cnt_finished_flows % 1000 == 0) {
        std::cout << Settings::cnt_finished_flows << " flows finished" << std::endl;
    }
    fflush(fout);
}
...
void stop_simulation_middle() {
    ...
    if (Settings::cnt_finished_flows >= target_flow_num) {
        ...
        std::cout << "被丢失的数据包数量为:" << Settings::dropped_pkt_sw_ingress << " " << Settings::dropped_pkt_sw_egress << std::endl;
...
void CalculateRoute(Ptr<Node> host) {
    ...
    for (int i = 0; i < (int)q.size(); i++) {
        ...
        for (auto it = nbr2if[now].begin(); it != nbr2if[now].end(); it++) {
            ...
            // If 'next' have not CalculateRoutebeen visited.
...
int main(int argc, char *argv[]) {
    ...
    topo2bdpMap[std::string("leaf_spine_128_100G_OS1")] = 104000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_128_100G_OS2")] = 104000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_256_100G_OS1")] = 104000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_256_100G_OS2")] = 104000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_512_100G_OS1")] = 104000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_512_200G_OS1")] = 204000;  // RTT=8320
    topo2bdpMap[std::string("leaf_spine_1024_100G_OS1")] = 104000;  // RTT=8320
    ...
    std::cout << "maxBdp:" << maxBdp << " irn_bdp_lookup:" << irn_bdp_lookup << std::endl;
    ...
    for (auto &pair : link_pairs) {
        Ptr<Node> probablyToRSw = n.Get(pair.first);
        Ptr<Node> probably_switch = n.Get(pair.second);
        if (probablyToRSw->GetNodeType() == 0 || probably_switch->GetNodeType() == 0) continue;
        Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(probablyToRSw);
        // host-switch link
        if (sw->m_isToR) {
            Ptr<SwitchNode> aggSw = DynamicCast<SwitchNode>(probably_switch);
            aggSw->m_isAgg = true;
        }
    }
    ...
    if (lb_mode == 3 || lb_mode == 6 || lb_mode == 9 || lb_mode == 10) {  // Conga, Letflow, Conweave
        std::cout << "Configuring Load Balancer's Switches" << endl;
        for (auto &pair : link_pairs) {
            Ptr<Node> probably_host = n.Get(pair.first);
            Ptr<Node> probably_switch = n.Get(pair.second);
                ...
                if (swSrc->m_isToR) {
                    ...
                    for (auto j = table1.begin(); j != table1.end(); j++) {
                        ...
                        for (auto next1 : nexts1) {
                            ...
                            uint32_t srcId = nodeSrc->GetId();
                            uint32_t aggId = next1->GetId();
                            Ptr<SwitchNode> swAgg = DynamicCast<SwitchNode>(next1);  // Agg switch

                            //std::cout << "Id的信息为:" << srcId << " " << outPort1 << " " << coreId << std::endl;
                            Settings::IdPortToId[GetSwitchPortKey(srcId, outPort1)] = aggId; 
                            if (nexts2.size() == 1 && nexts2[0]->GetId() == swDstId) {
                                ...
                                if (lb_mode == 10) {
                                    swSrc->m_mmu->m_gemmaRouting.m_GemmaRoutingTable[swDstId].insert(pathId);
                                }
                                continue;
                            }
                            ...
                            for (auto next2 : nexts2) {
                                ...
                                uint32_t coreId = next2->GetId();
                                Settings::IdPortToId[GetSwitchPortKey(aggId, outPort2)] = coreId; 
                                if (nexts3.size() == 1 && nexts3[0]->GetId() == swDstId) {
                                    ...
                                    if (lb_mode == 10) {
                                        swSrc->m_mmu->m_gemmaRouting.m_GemmaRoutingTable[swDstId]
                                            .insert(pathId);
                                ...
                                for (auto next3 : nexts3) {
                                    ...
                                    uint32_t aggId = next3->GetId();
                                    Settings::IdPortToId[GetSwitchPortKey(coreId, outPort3)] = aggId; 
                                    if (nexts4.size() == 1 && nexts4[0]->GetId() == swDstId) {
                                        Settings::is_fatTree = true;
                                        // this destination has 4-hop distance
                                        uint32_t outPort4 = nbr2if[next3][nexts4[0]].idx;
                                        Settings::IdPortToId[GetSwitchPortKey(aggId, outPort4)] = swDstId; 
                                        ...
                                        if (lb_mode == 10) {
                                            swSrc->m_mmu->m_gemmaRouting.m_GemmaRoutingTable[swDstId]
                                                .insert(pathId);
                                            swAgg->m_mmu->m_gemmaRouting.m_GemmaRoutingTable[swDstId]
                                                .insert(pathId);
                                        }   
                                        continue;
                                    } else {
        ...
        for (auto i = nextHop.begin(); i != nextHop.end(); i++) {  // every node
            if (i->first->GetNodeType() == 1) {
                ...
                if (lb_mode == 3) {
                    // conga_flowletTimeout = MicroSeconds(50);
                ...
                if (lb_mode == 10) {
                    sw->m_mmu->m_gemmaRouting.SetSwitchInfo(sw->m_isToR, sw->m_isAgg, sw->GetId());
                }
            }
        }
    ...
    string throughput_mon_file = uplink_mon_file.replace(uplink_mon_file.find("uplink"),6,"throughput");
