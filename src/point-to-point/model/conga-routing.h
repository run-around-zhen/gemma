...
class CongaRouting : public Object {
   ...
   public:
    ...
    static uint32_t nFlowletNum;
    static uint32_t nPauseFlowletNum;
    static uint32_t nPacketNum;
    static uint32_t nPauseFollowNum;                                                                 // number of flowlet's timeout
    ...
    uint32_t GetBestNoPausePath(uint32_t dstTorId, uint32_t qIndex);
    uint32_t GetPathCongestion(uint32_t dstToRId, uint32_t pathId);
