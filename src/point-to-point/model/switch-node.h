...
class SwitchNode : public Node {
   ...
   private:
    ...
    // Gemma (lb_mode = 9)
    uint32_t DoLbGemma(Ptr<const Packet> p, const CustomHeader &ch,
                           const std::vector<int> &nexthops);  // dummy
   ...
   public:
    ...
    Ptr<SwitchMmu> m_mmu;
    bool m_isToR;  
    bool m_isAgg;                               // true if ToR switch
    ...
    uint64_t GetAllBytesOutDev(uint32_t outdev);
};
