...
void SwitchMmu::SetPause(uint32_t port, uint32_t qIndex, uint32_t pause_time) {
    if (paused[port][qIndex] == false) {
        pausedTime[port][qIndex] = Simulator::Now().GetNanoSeconds();
        // std::cout << m_congaRouting.m_switch_id << " 交换机端口号:" << port << "被暂停了 " << Simulator::Now() << " " << pausedTime[port][qIndex] << std::endl;
    }
    paused[port][qIndex] = true;
    m_gemmaRouting.paused[port][qIndex] = true;
    m_congaRouting.paused[port][qIndex] = true;
...
void SwitchMmu::SetResume(uint32_t port, uint32_t qIndex) {
    if (paused[port][qIndex] == true) {
        pausedTime[port][qIndex] = 0;
    }
    paused[port][qIndex] = false;
    m_gemmaRouting.paused[port][qIndex] = false;
    m_congaRouting.paused[port][qIndex] = false;
    Simulator::Cancel(resumeEvt[port][qIndex]);
}
