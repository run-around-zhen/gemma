...
SwitchNode::SwitchNode() {
    ...
    m_mmu->m_gemmaRouting.SetSwitchSendCallback(MakeCallback(&SwitchNode::DoSwitchSend, this));
    m_mmu->m_gemmaRouting.SetSwitchSendToDevCallback(
        MakeCallback(&SwitchNode::SendToDevContinue, this));
...
/*------------------Gemma Dummy ----------------*/
uint32_t SwitchNode::DoLbGemma(Ptr<const Packet> p, const CustomHeader &ch,
                                  const std::vector<int> &nexthops) {
    return DoLbFlowECMP(p, ch, nexthops);  // flow ECMP (dummy)
    ...
    if (!foundGemmaNotifyTag) {
        // std::cout << "没有找到通知数据包 " << std::endl;
        return DoLbFlowECMP(p, ch, nexthops);  // flow ECMP (dummy)
    }
    uint32_t srcToRId = Settings::hostIp2SwitchId[ch.dip];
    uint32_t nowSwitchId = 0;
    uint pathId = gemmaNotifyTag.GetPathId();
    for (int i = 0; i < 3; i++) {
        uint32_t outDev = GemmaRouting::GetOutPortFromPath(pathId, i);
        if (i == 2) {
            nowSwitchId = Settings::IdPortToId[GemmaRouting::GetSwitchPortKey(srcToRId, outDev)];
        }else {
            srcToRId = Settings::IdPortToId[GemmaRouting::GetSwitchPortKey(srcToRId, outDev)];
        }
    }
    //std::cout << "目前在传输:" << nowSwitchId << "->" << srcToRId << std::endl; 
    for (int i = 0; i < nexthops.size(); i++) {   // 遍历发送方的端口,找到与之对应的中间交换机
        uint32_t portNum = nexthops[i];
        // std::cout << portNum << " ";
        uint64_t switchPortKey = GemmaRouting::GetSwitchPortKey(nowSwitchId, portNum);
        if (Settings::IdPortToId[switchPortKey] == srcToRId) {
           // std::cout << "找到端口为:" << portNum << std::endl;
            return portNum;
        }
    }
...
void SwitchNode::SendToDev(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (Settings::lb_mode == 10) {
        m_mmu->m_gemmaRouting.RouteInput(p,ch);
        return;
    }
...
int SwitchNode::GetOutDev(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (Settings::lb_mode == 0 || control_pkt && (Settings::lb_mode != 10)) {  // control packet (ACK, NACK, PFC, QCN)
    ...
    switch (Settings::lb_mode) {
        ...
        case 10:
            return DoLbGemma(p, ch, nexthops);
        default:
...
void SwitchNode::DoSwitchSend(Ptr<Packet> p, CustomHeader &ch, uint32_t outDev, uint32_t qIndex) {
    ...
    uint64_t flowKey = m_mmu->m_gemmaRouting.GetFlowKey(ch.sip, ch.dip, ch.udp.sport, ch.udp.dport);
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p) {
    ...
    if (qIndex != 0) {
        uint32_t inDev = t.GetFlowId();
        if (inDev != Settings::CONWEAVE_CTRL_DUMMY_INDEV && inDev != Settings::Gemma_CTRL_DUMMY_INDEV) {
    ...
    // if (t.GetFlowId() == Settings::CONWEAVE_CTRL_DUMMY_INDEV && !m_isToR) {
    //     std::cout << "conweave数据包:" <<  p->GetSize() << " " << m_isToR << std::endl;
    // }
    // if (t.GetFlowId() == Settings::Gemma_CTRL_DUMMY_INDEV) {
    //     std::cout << "gemma数据包:" <<  p->GetSize() << " " << m_isToR << std::endl;
    // }
    m_allBytes[ifIndex] += p->GetSize();
    // if (!m_isToR) std::cout << "不是中间交换机" << std::endl;
    // std::cout << "是否是中间交换机: " << m_isToR << std::endl;
    if (t.GetFlowId() != Settings::CONWEAVE_CTRL_DUMMY_INDEV && t.GetFlowId() != Settings::Gemma_CTRL_DUMMY_INDEV) {
...
void SwitchNode::AddTableEntry(Ipv4Address &dstAddr, uint32_t intf_idx) {
    ...
    m_mmu->m_gemmaRouting.m_ipTable[dip] = intf_idx;
    m_mmu->m_conweaveRouting.m_ipTable[dip] = intf_idx;
}

void SwitchNode::ClearTable() { m_rtTable.clear(); }

uint64_t SwitchNode::GetAllBytesOutDev(uint32_t outdev) {
    assert(outdev < pCnt);
    return m_allBytes[outdev];
}
