...
class Settings {
   public:
    ...
    static const uint32_t Gemma_CTRL_DUMMY_INDEV = 88888889;  // just arbitrary
    ...
    static uint64_t maxRepeatFlowNum;
    static bool is_fatTree;
    ...
    static std::map<uint64_t, uint64_t> flowKeyNum;  
    static std::map<uint64_t, uint64_t> IdPortToId;  // 根据id+端口号获取到下一个id
    static std::map<uint64_t, uint32_t> m_sendPakcetNum;       // 指定交换机id+端口号发送的数据包
    static std::map<uint64_t, uint32_t> m_receivePacketNum;    // 对应的交换机id+端口号接收到的数据包
