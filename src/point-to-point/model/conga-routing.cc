...
uint32_t CongaRouting::nPacketNum = 0;
uint32_t CongaRouting::nFlowletNum = 0;
uint32_t CongaRouting::nPauseFlowletNum = 0;
uint32_t CongaRouting::nPauseFollowNum = 0;
CongaRouting::CongaRouting() {
    ...
    m_flowletTimeout = Time(MicroSeconds(50));
    m_quantizeBit = 3;
    m_alpha = 0.2;
}
...
void CongaRouting::RouteInput(Ptr<Packet> p, CustomHeader ch) {
    ...
    if (m_isToR) {     // ToR switch
        ...
        // if (paused[outPort][ch.udp.pg]) {
        //     CongaRouting::nPauseFollowNum ++;
        //     // std::cout << "挑选的路径被pfc暂停辣" << std::endl;
        // }
...
uint32_t CongaRouting::GetPathCongestion(uint32_t dstToRId,uint32_t pathId) {
    uint32_t localCongestion = 0;
    uint32_t remoteCongestion = 0;
...
uint32_t CongaRouting::GetBestNoPausePath(uint32_t dstToRId, uint32_t qIndex) {
    auto pathItr = m_congaRoutingTable.find(dstToRId);
    assert(pathItr != m_congaRoutingTable.end() && "Cannot find dstToRId from ToLeafTable");
    std::set<uint32_t>::iterator innerPathItr = pathItr->second.begin();
    ...
    std::vector<uint32_t> candidatePaths;
    uint32_t minCongestion = CONGA_NULL;
    for (uint32_t i = 0; i < pathItr->second.size(); i++) {
        // get info of path
        uint32_t pathId = *innerPathItr;
        ...
        auto outPort = GetOutPortFromPath(pathId, 0);  // outPort from pathId (TxToR)
        if (paused[outPort][qIndex]) {
            // std::cout << m_switch_id << " 该端口被pfc暂停了" << outPort << std::endl;
            std::advance(innerPathItr, 1);
            continue;
        }
